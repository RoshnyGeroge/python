...
def hello():
    ...
    return "Hello World! Roshny Here"
...
@app.route('/metrics')
def metrics():
    response = app.response_class(response=json.dumps({"status":"success", "code":0,
                                                       "data": {"UserCount":140,"userActive":23}}),
                                  status =200,
                                  mimetype='application/json'
                                  )
...
""" Dockerization of Flask app
 ...
 4. create docker image
   ...
   b) docker build -t go-helloworld . # creates docker image. . stands for current directory
...
kubectl create deploy go-helloworld --image=roshg/py-helloworld:v1.0.0
...
```
Access the application in the local machine on  http://127.0.0.1:6111/ 
